...
# 2. MODEL DEFINITION
# ==========================================
class lfistaLayer(nn.Module):
    """
    A single unrolled iteration of FISTA.
    """
    def __init__(self, n, m, A, L, lambda_val, alpha_init):
        super(lfistaLayer, self).__init__()
        ...
        W2_init = (A.T / L).detach()
        W1_init = (torch.eye(n, device=device) - (A.T @ A) / L).detach()
        ...
        self.alpha = nn.Parameter(alpha_init)    # Initialized according to FISTA theory
        
    def forward(self, x_k, x_k_prev, y):
        
        # Momentum update
        z = x_k + self.alpha * (x_k - x_k_prev)
        
        # Learned linear step
        temp = z @ self.W1.T + y @ self.W2.T
        
        # Soft Thresholding
        theta = torch.abs(self.theta)
        x_new = torch.sign(temp) * torch.relu(torch.abs(temp) - theta)
        
        return x_new

class lfista(nn.Module):
    def __init__(self, A, num_layers=15, lambda_val=0.01):
        ...
        # Compute FISTA momentum coefficients for initialization
        alphas = []
        t_prev = torch.tensor(1.0, device=device)
        for i in range(num_layers):
            t = torch.tensor((1 + torch.sqrt(1 + 4 * t_prev**2)) / 2, device=device)
            alpha = (t_prev - 1) / t
            alphas.append(torch.tensor(alpha, device=device).detach())
            t_prev = t
        
        # Create the Unrolled Layers
        self.layers = nn.ModuleList([
            lfistaLayer(n, m, A, L, lambda_val, alphas[i]) for i in range(num_layers)
        ])

    def forward(self, y):
        batch_size = y.shape[0]
        # Initialize x with 0
        x_k = torch.zeros(batch_size, N_SIGNAL, device=device)
        x_k_prev = torch.zeros(batch_size, N_SIGNAL, device=device)
        '''
        # Initialize momentum terms
        t_k = torch.tensor(1.0, device=device)
        t_k_prev = torch.tensor(1.0, device=device)
        '''
        
        # Pass through layers
        for layer in self.layers:
            x_k = layer(x_k, x_k_prev, y)
            x_k_prev = x_k
            
        return x_k
    
# Initialize Model
model = lfista(A, NUM_LAYERS).to(device)
...
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=25, gamma=0.80)
...
plt.stem(np.arange(N_SIGNAL), x_est_np[:N_SIGNAL], linefmt='b--', markerfmt='bx', label='lfista')
plt.title(f"Sample #{sample_idx} Recon", fontsize=10, fontweight='bold')
plt.legend()
plt.grid(True, alpha=0.3)

plt.tight_layout(pad=2.0)
plt.savefig('lfista_stats.png', dpi=150)
print("Figure saved as 'lfista_stats.png'")
plt.show()