...
class PristineDataset(Dataset):
    """
    Loads raw images from disk and resizes them to 64*64.
    All augmentations happen GPU-side in the training loop.
    """

    def __init__(self, folder_path: str = "TinyImageNet/train"):
...
def train(args):
    ...
    if is_main:
        print("=" * 60)
        print("  Modified ResNet10 Based Asymmetric Contrastive Training")
    ...
    try:
        dataset = PristineDataset(folder_path=dataset_path)
        sampler = None
        if is_distributed:
        ...
        model = ResNet10(embedding_dim=128).to(device)
